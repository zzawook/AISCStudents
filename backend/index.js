...
addHashTag = (articleId, hashTag) => {   
    if ((hashTag.includes(`#${articleId}`))) {
        ...
        con.connect(function (err) {
            ...
            let sql = noHashTag? `update articleDB set articleDB.hashTag=concat(articleDB.hashTag, '#?') where articleDB.id=?`:`update articleDB set articleDB.hashTag=concat(articleDB.hashTag, ',#?') where articleDB.id=?`
            con.query(sql, [articleId, articleId], function (err, result) {
                if (err) {
                    throw err;
                    con.end();
                    console.log(err)
